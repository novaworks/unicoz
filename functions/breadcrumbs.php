...
function nova_breadcrumbs( $args = '' ) {
	$args = wp_parse_args( $args, array(
		...
		'show_on_front'     => false,
		'labels'            => array(
			'home'      => esc_html__( 'Home', 'unicoz' ),
			'archive'   => esc_html__( 'Archives', 'unicoz' ),
			'blog'      => esc_html__( 'Blog', 'unicoz' ),
			'search'    => esc_html__( 'Search results for', 'unicoz' ),
			'not_found' => esc_html__( 'Not Found', 'unicoz' ),
			'author'    => esc_html__( 'Author Archives:', 'unicoz' ),
			'day'       => esc_html__( 'Daily Archives:', 'unicoz' ),
			'month'     => esc_html__( 'Monthly Archives:', 'unicoz' ),
			'year'      => esc_html__( 'Yearly Archives:', 'unicoz' ),
		),
	) );
	...
	elseif ( is_day() ) {
		$items[] = sprintf(
			$item_text_tpl,
			sprintf( esc_html__( '%s %s', 'unicoz' ), $args['labels']['day'], get_the_date() )
		);
	} // Month archive
	elseif ( is_month() ) {
		$items[] = sprintf(
			$item_text_tpl,
			sprintf( esc_html__( '%s %s', 'unicoz' ), $args['labels']['month'], get_the_date( 'F Y' ) )
		);
	} // Year archive
	elseif ( is_year() ) {
		$items[] = sprintf(
			$item_text_tpl,
			sprintf( esc_html__( '%s %s', 'unicoz' ), $args['labels']['year'], get_the_date( 'Y' ) )
		);
	} // Archive
	else {
		$items[] = sprintf(
