		...
		<div class="nova-blog-posts post-display-block">
			...
			<div class="nova-blog-posts__content<?php if(has_post_thumbnail()): ?> has-thumbnail<?php endif?>">
						...
						?>
							...
							<div class="entry-meta__cat">
								<?php
										...
										foreach( $categories as $category ) {
												$output .= '<a href="' . esc_url( get_category_link( $category->term_id ) ) . '" alt="' . esc_attr( sprintf( __( 'View all posts in %s', 'unicoz' ), $category->name ) ) . '">' . esc_html( $category->name ) . '</a>' . $separator;
										}
										echo trim( $output, $separator );
					...
					if( $excerpt_limit ) {
						if($excerpt_length > 0){
								echo sprintf(
										...
										unicoz_excerpt(intval( $excerpt_length ))
								);
						}
					}else {
						echo sprintf(
				...
				<div class="nova-blog-posts__readmore-wrap">
					<a class="nova-blog-posts__readmore link-animation-icon" href="<?php echo(esc_url(get_permalink())); ?>"><?php esc_html_e( 'Read more', 'unicoz') ?><i class="inova ic-arrow-right"></i></a>
				</div>
			</div>
		</div>
</article>
