...
?>
		<div class="off-canvas-wrapper">
				...
				<div class="nova-offcanvas sizeguide-canvas site-canvas-menu off-canvas position-right" id="SizeGuide" data-off-canvas data-transition="overlap">
					<h2 class="title"><?php echo esc_html__( 'Size Guidelines', 'unicoz' );?></h2>
				...
				<?php if ( NOVA_WOOCOMMERCE_IS_ACTIVE ) : ?>
					<?php if ( Nova_OP::getOption('header_cart') == 1 &&  Nova_OP::getOption('header_cart_action') == 'mini-cart') : ?>
						<div class="nova-offcanvas minicart-canvas site-canvas-menu off-canvas position-right" id="MiniCartCanvas" data-off-canvas data-transition="overlap">
							<h2 class="title"><?php echo esc_html__( 'Shopping Cart', 'unicoz' );?><span class="nova_js_count_bag_item_canvas count-item-canvas"><?php echo esc_html(WC()->cart->get_cart_contents_count()); ?></span></h2>
				...
				<?php endif; ?>
				<?php if ( NOVA_WOOCOMMERCE_IS_ACTIVE ) : ?>
					<?php if ( Nova_OP::getOption('header_user_account') == 1 &&  Nova_OP::getOption('header_user_action') == 'modal'  && !is_user_logged_in() ) : ?>
